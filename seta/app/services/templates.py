...
async def apply(
    ...
    *,
    ...
    # Тот же запрет на дубли, что у переходов состояния: десять нажатий одной
    # кнопки дают одно поручение. Признак двойного нажатия — не только статус,
    # но и свежесть: еженедельный отчёт, выданный в прошлый понедельник и так
    # и не принятый, не делает сегодняшний отчёт дубликатом. Поэтому окно
    # короткое — ровно на длину замешательства, а не на неделю.
    twin = (
        await session.execute(
            select(Task).where(
